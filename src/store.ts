...
import { type Element, type Elements, isNode } from "@braks/vue-flow";
...
const addSticker = (x: number, y: number) => {
  const transform = state.vueFlowStore.getTransform();
  ...
  state.vueFlowStore.addNodes([
    {
      ...
      position: { x: x / transform.zoom, y: y / transform.zoom },
      class: "light",
    },
  ]);
...
const zoomIn = () => {
  state.currentAction = Action.ZoomIn;
  state.vueFlowStore.zoomIn();
  setTimeout(function () {
    state.currentAction = Action.None;
  }, 150);
};
...
const zoomOut = () => {
  state.currentAction = Action.ZoomOut;
  state.vueFlowStore.zoomOut();
  setTimeout(function () {
    state.currentAction = Action.None;
  }, 150);
};
...
const shuffle = () => {
  state.currentAction = Action.Shuffle;
